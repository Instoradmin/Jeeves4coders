...
import { ConfigurationManager, AccountConfig, SSOConfig } from './configManager';
...
export class ConfigurationWebview {
    ...
    public async show() {
        ...
        this.panel = vscode.window.createWebviewPanel(
            'jeeves4coders.config',
            'Jeeves4coders Configuration',
    ...
    private async handleConnectGitHub(data: any) {
        try {
    ...
    private async handleConnectJIRA(data: any) {
        try {
            const config: AccountConfig = {
                type: 'jira',
                connected: true,
    ...
    private async handleConnectConfluence(data: any) {
        try {
            const config: AccountConfig = {
                type: 'confluence',
                connected: true,
    ...
    private async handleDisconnect(accountType: string) {
        try {
            if (accountType === 'sso') {
                await this.configManager.clearSSO();
                vscode.window.showInformationMessage('SSO disconnected successfully!');
            } else {
                await this.configManager.clearAccountConfig(accountType as any);
    ...
    private async handleTestConnection(accountType: string) {
        try {
    ...
    private async refreshWebviewData() {
        if (!this.panel) return;
