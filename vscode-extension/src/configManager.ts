...
export class ConfigurationManager {
     ...
     */
    async initiateSSO(): Promise<boolean> {
        try {
            ...
            if (!config.clientId) {
                ...
                await vscode.workspace.getConfiguration('jeeves4coders').update(
                    'oauth.clientId', 
                    clientId, 
            ...
            if (success) {
                ...
                vscode.window.showInformationMessage(
                    'Please complete the authentication in your browser. The callback server is running on localhost:8080.'
                );
                return true;
            }
     ...
     */
    private async startOAuthServer(expectedState: string): Promise<boolean> {
     ...
     */
    async validateAndRefreshSSO(): Promise<boolean> {
        try {
            const config = await this.getSSOConfig();
     ...
     * Refresh SSO tokens
     */
    private async refreshSSO(): Promise<boolean> {
        try {
            const config = await this.getSSOConfig();
     ...
     */
    async getAccountConfig(type: 'github' | 'jira' | 'confluence'): Promise<AccountConfig> {
        const config = vscode.workspace.getConfiguration('jeeves4coders');
     ...
     */
    async storeAccountConfig(config: AccountConfig): Promise<void> {
     ...
     */
    async clearAccountConfig(type: 'github' | 'jira' | 'confluence'): Promise<void> {
        const config = vscode.workspace.getConfiguration('jeeves4coders');
